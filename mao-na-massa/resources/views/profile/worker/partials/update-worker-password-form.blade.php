<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            {{ __('Atualize a sua Senha') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Verifique se a sua senha está nos padrões de segurança') }}
        </p>
    </header>

    <form method="post" action="{{ route('worker.password.update') }}" class="mt-6 space-y-6">
        @csrf
        @method('put')
